...
vi.mock('./lib/api', () => {
  ...
  return { ApiRequestError, hasAccessToken: vi.fn(), setAccessToken: vi.fn(), api: { me: vi.fn(), health: vi.fn(), login: vi.fn() } };
});
...
describe('local session recovery', () => {
  ...
  it('replaces a cached auth error after a successful login', async () => {
    ...
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    await client.fetchQuery({ queryKey: ['me'], queryFn: () => Promise.reject(new ApiRequestError('Expired', 401)) }).catch(() => {});
    render(<QueryClientProvider client={client}><App /></QueryClientProvider>);
