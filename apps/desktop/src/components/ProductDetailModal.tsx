...
export function ProductDetailModal({ product: initialProduct, onClose, canManage = false }: { product: ProductListItem; onClose: () => void; canManage?: boolean }) {
  ...
  const [draft, setDraft] = useState({ images, videoUrl: product.videoUrl ?? '' });
  const cache = useQueryClient();
  async function saveMedia() {
    setSaving(true); setError('');
    try {
      const media = await api.updateProductMedia(product.id, draft);
      setProduct(current => ({...current, ...media, imageUrl: media.images[0] ?? ''}));
      setActiveImage(0); setEditing(false);
  ...
  return <Modal title="Fiche produit" subtitle={`${product.sku} · catalogue et stock partagé`} onClose={() => { if (!saving && !uploading) onClose(); }} size="wide">
    <div className="p-5 sm:p-6">
        ...
        <div className="relative grid min-h-60 place-items-center overflow-hidden bg-linear-to-br from-brand-soft via-white to-pink-50 dark:from-[#392830] dark:via-[#282428] dark:to-[#302630] p-6">
          ...
          <div className="w-full pt-6">
            {activeImage === images.length && product.videoUrl
              ? <video key={product.videoUrl} src={product.videoUrl} controls preload="metadata" className="max-h-64 w-full rounded-xl bg-black" />
              : images[activeImage] ? <img src={images[activeImage]} alt={product.name} className="h-56 w-full object-contain" />
              : <span className="grid h-56 place-items-center text-5xl font-bold text-brand">{product.name[0]}</span>}
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {images.map((url, index) => <button key={url} type="button" aria-label={`Voir l’image ${index + 1}${index === 0 ? ', à la une' : ''}`} aria-pressed={activeImage === index} onClick={() => setActiveImage(index)} className={`size-12 overflow-hidden rounded-lg border-2 bg-white dark:bg-[#302b2f] ${activeImage === index ? 'border-brand' : 'border-line'}`}><img src={url} alt="" className="size-full object-contain" /></button>)}
              {product.videoUrl && <button type="button" aria-pressed={activeImage === images.length} onClick={() => setActiveImage(images.length)} className={`rounded-lg border-2 px-2 text-xs ${activeImage === images.length ? 'border-brand text-brand' : 'border-line'}`}>▶ Vidéo</button>}
            </div>
          </div>
        </div>
        <div className="flex flex-col justify-between p-5 sm:p-7">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[9px] font-bold ${product.retailVisible ? 'bg-brand-soft text-brand' : 'bg-stone-100 dark:bg-[#302b2f] text-muted'}`}>
      ...
      {canManage && <div className="mt-4">
        {!editing ? <button type="button" className="rounded-lg border border-line px-4 py-2 text-xs font-bold text-brand" onClick={() => {setDraft({images, videoUrl: product.videoUrl ?? ''}); setEditing(true); setError('');}}>Gérer la galerie & vidéo</button> : <>
          <ProductMediaEditor value={draft} onChange={setDraft} disabled={saving} onBusyChange={setUploading} />
          {error && <p role="alert" className="mb-3 text-xs text-rose-600 dark:text-rose-300">{error}</p>}
          <div className="flex gap-3"><button type="button" disabled={saving || uploading} onClick={() => void saveMedia()} className="rounded-lg bg-brand px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{saving ? 'Enregistrement…' : 'Enregistrer les médias'}</button><button type="button" disabled={saving || uploading} onClick={() => setEditing(false)} className="text-xs text-muted">Annuler</button></div>
        </>}
      </div>}
