...
export const documentStatuses: Record<string, string> = { ORDERED: 'Commandée', CONFIRMED: 'Confirmée', PAID: 'Payée', PARTIALLY_PAID: 'Partiellement payée', DELIVERED: 'Livrée', FULFILLED: 'Livrée', CANCELED: 'Annulée', CANCELLED: 'Annulée', REFUNDED: 'Remboursée', RECEIVED: 'Reçue', PARTIALLY_RECEIVED: 'Partiellement reçue', DRAFT: 'Brouillon' };

const paymentMethods: Record<string, string> = { CASH: 'Espèces', CHECK: 'Chèque', CARD: 'Carte', TRANSFER: 'Virement', CREDIT: 'Crédit', COD: 'Paiement à la livraison', OTHER: 'Autre' };
...
export function InvoiceDetailModal({ kind, id, onClose, surface = 'invoice' }: { kind: 'sale' | 'purchase'; id: string; onClose: () => void; surface?: 'order' | 'invoice' }) {
  ...
  const paidProgress = value && value.total > 0 ? Math.min(100, Math.round((value.amountPaid / value.total) * 100)) : 0;
  return <Modal title={surface === 'order' ? 'Détails de la commande' : 'Détails de la facture'} subtitle={value?.documentNumber ?? 'Chargement du document…'} onClose={onClose} size="wide">
    ...
    {value && <div className="space-y-5 p-5 sm:p-6">
      <section className="overflow-hidden rounded-2xl border border-line bg-white">
        <div className="flex flex-wrap items-start justify-between gap-4 bg-linear-to-r from-brand-soft/80 via-white to-pink-50 px-5 py-5 sm:px-6">
          ...
          <div className="text-right"><span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-bold ${statusStyle(value.status)}`}><i className="size-1.5 rounded-full bg-current" />{documentStatuses[value.status] ?? value.status}</span><p className="mt-2 flex items-center justify-end gap-1.5 text-[10px] text-muted"><CalendarDays size={13} />{new Date(value.issuedAt).toLocaleString('fr-FR', { dateStyle: 'long', timeStyle: 'short' })}</p></div>
        </div>
        <div className="grid border-t border-line md:grid-cols-[1.1fr_.9fr]">
          <div className="p-5 sm:p-6 md:border-r md:border-line">
            <p className="text-[9px] font-bold uppercase tracking-widest text-muted">{kind === 'sale' ? 'Client' : 'Fournisseur'} · coordonnées enregistrées à l’émission</p>
            <div className="mt-4 flex items-start gap-3">
              <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand-soft text-brand">{kind === 'sale' ? <UserRound size={18} /> : <Building2 size={18} />}</span>
              <div className="min-w-0"><h3 className="text-base font-bold text-ink">{value.partnerName}</h3><div className="mt-3 grid gap-2 text-[11px] text-muted">
                <p className="flex items-start gap-2"><Phone size={14} className="mt-0.5 shrink-0 text-brand" /><span>{value.partner?.phone || 'Téléphone non renseigné'}</span></p>
                <p className="flex items-start gap-2"><Mail size={14} className="mt-0.5 shrink-0 text-brand" /><span className="break-all">{value.partner?.email || 'Email non renseigné'}</span></p>
                <p className="flex items-start gap-2"><MapPin size={14} className="mt-0.5 shrink-0 text-brand" /><span className="whitespace-pre-wrap">{value.partner?.address || 'Adresse non renseignée'}</span></p>
              </div></div>
            </div>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-[1fr_340px]">
        <div>
          {value.notes ? <div className="rounded-2xl border border-brand/10 bg-brand-soft/55 p-4"><p className="text-[9px] font-bold uppercase tracking-widest text-brand">Note interne</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-ink">{value.notes}</p></div> : <div className="rounded-2xl border border-dashed border-line p-4 text-xs text-muted">Aucune note ajoutée à ce document.</div>}
        </div>
        <dl className="rounded-2xl border border-line bg-surface p-4 text-xs [&>div]:flex [&>div]:justify-between [&>div]:gap-4 [&>div]:py-1.5 [&_dt]:text-muted [&_dd]:font-semibold [&_dd]:text-ink">
          <div><dt>Sous-total</dt><dd>{money.format(value.subtotal)}</dd></div>
          <div><dt>Remise</dt><dd>- {money.format(value.discountTotal)}</dd></div>
          <div><dt>Livraison</dt><dd>{money.format(value.shippingTotal)}</dd></div>
          <div><dt>Taxes</dt><dd>{money.format(value.taxTotal)}</dd></div>
          <div className="mt-2 border-t border-line pt-3! text-sm"><dt className="font-bold text-ink!">Total</dt><dd className="text-base text-brand!">{money.format(value.total)}</dd></div>
        </dl>
      </section>
