  ...
  return <>
    ...
    <main>
      ...
      <section className="catalog" id="catalogue"><div className="store-section-title"><div><h2>{search ? `Résultats pour « ${search} »` : category === "ALL" ? "Nos meilleures ventes" : categoryCards.find((item) => item.id === category)?.name}</h2><p>Le catalogue publié depuis GlowCare Business, avec le stock disponible en direct.</p></div>{(search || category !== "ALL") && <button onClick={() => { setSearch(""); setCategory("ALL"); }}>Effacer les filtres <X size={15} /></button>}</div><div className="product-grid">
        {shownProducts.map((product, index) => <article className="store-product" key={product.variantId}><div className={`product-visual shade-${index % 5}`}>{product.imageUrl ? <img src={product.imageUrl} alt={product.name} loading="lazy" /> : <span className={`cosmetic-shape shape-${index % 4}`}><i>{product.brand.slice(0, 1)}</i></span>}<button className="heart" aria-label={`Ajouter ${product.name} aux favoris`}><Heart size={17} /></button><button className="detail-trigger" onClick={() => setSelectedProduct(product)}>Voir les détails</button>{product.available <= 0 && <b>Épuisé</b>}{product.available > 0 && product.available <= product.lowStockThreshold && <b>Dernières pièces</b>}</div><p>{product.brand}</p><button className="product-title" onClick={() => setSelectedProduct(product)}><h3>{product.name}</h3></button><small>{product.sku}</small><div className="rating"><span>{[0, 1, 2, 3, 4].map((star) => <Star key={star} size={12} fill="currentColor" />)}</span><small>({120 + index * 17})</small></div><div className="product-buy"><span><strong>{price.format(product.retailPrice)}</strong>{product.compareAtPrice && product.compareAtPrice > product.retailPrice ? <del>{price.format(product.compareAtPrice)}</del> : null}</span><button disabled={product.available <= 0} onClick={() => add(product)}><ShoppingCart size={15} /> Ajouter</button></div></article>)}
    ...
    {selectedProduct && <div className="product-modal-backdrop" onMouseDown={() => setSelectedProduct(null)}><section className="product-modal" onMouseDown={(event) => event.stopPropagation()}><button className="product-modal-close" onClick={() => setSelectedProduct(null)} aria-label="Fermer"><X size={20} /></button><div className="product-modal-image">{selectedProduct.imageUrl ? <img src={selectedProduct.imageUrl} alt={selectedProduct.name} /> : <span className="cosmetic-shape"><i>{selectedProduct.brand.slice(0, 1)}</i></span>}</div><div className="product-modal-copy"><p>{selectedProduct.brand}</p><h2>{selectedProduct.name}</h2><div className="rating"><span>{[0, 1, 2, 3, 4].map((star) => <Star key={star} size={14} fill="currentColor" />)}</span><small>Produit authentique</small></div><div className="modal-price"><strong>{price.format(selectedProduct.retailPrice)}</strong>{selectedProduct.compareAtPrice && selectedProduct.compareAtPrice > selectedProduct.retailPrice ? <del>{price.format(selectedProduct.compareAtPrice)}</del> : null}</div><p className="product-description">{selectedProduct.description || 'Produit cosmétique sélectionné avec soin par GlowCare.'}</p><small className="product-reference">Référence : {selectedProduct.sku} · {selectedProduct.available} en stock</small><button className="modal-add" disabled={selectedProduct.available <= 0} onClick={() => { add(selectedProduct); setSelectedProduct(null); }}><ShoppingCart size={17} /> Ajouter au panier</button></div></section></div>}
