...
export class AuthService {
  private readonly secret = process.env.AUTH_SECRET ?? 'development-secret-change-me';
  ...
  verify(token: string): TokenPayload {
    ...
    if (!encodedPayload || !signature) throw new UnauthorizedException('Session invalide.');
  ...
  async authenticate(token: string): Promise<AuthUser> {
    ...
    const user = result.rows[0];
    if (!user) throw new UnauthorizedException('Ce compte est inactif ou introuvable.');
    return user;
  }

  async listUsers(): Promise<TeamUser[]> {
    const result = await this.db.query<UserRow & { active: boolean; createdAt: string }>(
      ...
      const result = await this.db.query<UserRow & { active: boolean; createdAt: string }>(
        `INSERT INTO users
         ...
         RETURNING id, email, display_name AS "displayName", role, active,
           created_at::text AS "createdAt"`,
        [DEFAULT_ORGANIZATION_ID, input.email, input.displayName, input.password, input.role],
      );
  ...
  async updateUser(id: string, input: UpdateUserInput): Promise<TeamUser> {
    const result = await this.db.query<UserRow & { active: boolean; createdAt: string }>(
      `UPDATE users SET
       ...
       RETURNING id, email, display_name AS "displayName", role, active,
         created_at::text AS "createdAt"`,
      [DEFAULT_ORGANIZATION_ID, id, input.role ?? null, input.active ?? null, input.password ?? null],
    );
    if (!result.rows[0]) throw new UnauthorizedException('Utilisateur introuvable.');
    return result.rows[0];
  }
